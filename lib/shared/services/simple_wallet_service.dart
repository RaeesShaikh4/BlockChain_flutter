  ...
  Future<List<TransactionInfo>> getTransactionHistory({
    ...
    try {
      ...
      for (int i = 0; i < 5; i++) {
        ...
        final amount = random.nextDouble() * 5; // Random amount 0-5 ETH
        final timestamp = DateTime.now().subtract(Duration(days: i));
        
        transactions.add(TransactionInfo(
          hash: '0x${random.nextInt(0xffffffff).toRadixString(16).padLeft(8, '0')}',
          from: isIncoming ? '0x${List.generate(20, (i) => random.nextInt(256).toRadixString(16).padLeft(2, '0')).join()}' : _address!,
          to: isIncoming ? _address! : '0x${List.generate(20, (i) => random.nextInt(256).toRadixString(16).padLeft(2, '0')).join()}',
